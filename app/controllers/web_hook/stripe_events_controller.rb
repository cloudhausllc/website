  ...
  def create
    ...
    if @stripe_event.event_type == 'customer.created'
      ...
      @stripe_event[:data]['sources']['data'].each do |card|
        card = user.payment_methods.where(stripe_card_id: card['id']).first_or_create
        card.update({brand: card['brand'],
      ...
      user.payment_methods.where(stripe_card_id: @stripe_event['data']['card'],
                                 brand: @stripe_event['data']['brand'],
                                 exp_month: @stripe_event['data']['exp_month'],
                                 exp_year: @stripe_event['data']['exp_year'],
                                 last4: @stripe_event['data']['last4']).first_or_create
      @stripe_event.mark_as_processed
      ...
      Plan.find_by_stripe_plan_id(@stripe_event['data']['id']).update(active: false)
    elsif @stripe_event.event_type == 'plan.updated'
      Plan.find_by_stripe_plan_id(@stripe_event['data']['id']).update({
